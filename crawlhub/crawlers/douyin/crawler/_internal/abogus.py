"""
ABogus Parameter Generator
===========================
Pure Python implementation of Douyin's a_bogus URL signing parameter.
Adapted from TikTokDownloader (https://github.com/JoeanAmier/TikTokDownloader).

Algorithm: SM3 (Chinese national hash) + RC4 encryption + custom Base64 encoding.
Dependencies: gmssl

Usage:
    from crawler.abogus import ABogus
    ab = ABogus(user_agent="Mozilla/5.0 ...")
    a_bogus_value = ab.get_value(url_params_string, "GET")
"""
...
class ABogus:
    """Pure Python a_bogus parameter generator.

    Uses SM3 hash + RC4 encryption to produce signed URL parameters
    that Douyin's server validates.
    """
    ...
    def __init__(self, user_agent: str = None, platform: str = None):
        self.chunk = []
        self.size = 0
        self.reg = self.__reg[:]
    ...
    @classmethod
    def generate_string_1(cls, rn1=None, rn2=None, rn3=None):
        return (
            cls.from_char_code(*cls.list_1(rn1))
            + cls.from_char_code(*cls.list_2(rn2))
            + cls.from_char_code(*cls.list_3(rn3))
        )
    ...
    def sm3_to_array(cls, data):
        """Compute SM3 hash and return as integer array."""
    ...
    def get_value(self, url_params, method="GET", start_time=0, end_time=0,
        ...
        """Generate a_bogus parameter value.

        Args:
            url_params: URL query string (already encoded) or dict.
            method:     HTTP method ("GET" or "POST").

        Returns:
            str: The a_bogus parameter value.
        """
